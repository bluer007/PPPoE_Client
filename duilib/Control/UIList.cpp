...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1),m_iCurSelActivate(-1)		//古月：m_iCurSelActivate(-1)是根据alberl教程添加的，用在alberl新添加的函数CListUI::SelectItemActivate中，用于记录双击列表确认的项的索引号
{
...
bool CListUI::SelectItemActivate(int iIndex)		//古月：这个函数是根据alberl教程添加的，用于保存双击列表确认的项的索引号
{
...
bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
{
    ...
    //if( !pControl->IsVisible() ) return false;		//古月：按照alberl教程，把这两句给注释掉，达到了就算该控件不可见，uenabled，该控件仍然可以实现SelectItem效果，方便用户编程,因为mfc等的控件都是这样的逻辑的
    //if( !pControl->IsEnabled() ) return false;			//古月：按照alberl教程，把这两句给注释掉，达到了就算该控件不可见，uenabled，该控件仍然可以实现SelectItem效果，方便用户编程
...
void CListTextElementUI::DoEvent(TEventUI& event)
{
    ...
    // When you hover(古月：hover徘徊的意思) over a link
    if( event.Type == UIEVENT_SETCURSOR ) {
