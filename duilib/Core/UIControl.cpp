...
void CControlUI::DoEvent(TEventUI& event)
{
	...
	{
		m_pManager->SendNotify(this, DUI_MSGTYPE_DBCLICK, event.wParam, event.lParam);	//古月：这段代码是按照alberl添加的，原来duilib并没有的
		//return;		//古月：按照alberl说法，不返回是为了让这个双击消息也传给父窗口，不仅仅由子控件负责，所以当notify函数收到DUI_MSGTYPE_DBCLICK_T("dbclick")) 时，父窗口和子控件的notify函数都会收到，如果不想让父窗口收到，可以直接return。
	}
	///////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if( m_pParent != NULL ) m_pParent->DoEvent(event);		//古月：安照字面理解，就是其他的消息（control不处理的），就让控件的父窗口（m_pParent）来负责处理
}
