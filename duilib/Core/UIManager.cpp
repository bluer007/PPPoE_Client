...
void CPaintManagerUI::MessageLoop()
{
    ...
    while( ::GetMessage(&msg, NULL, 0, 0) ) {
        if( !CPaintManagerUI::TranslateMessage(&msg) ) {		//古月：据我追踪发现该CPaintManagerUI::TranslateMessage会调用CPaintManagerUI::PreMessageHandler来预处理全部窗口的系统消息，然后CPaintManagerUI::PreMessageHandler会调用MessageHandler（应该是CPaintManagerUI类的）来处理duilib所能处理的消息
...
bool CPaintManagerUI::TranslateMessage(const LPMSG pMsg)
{
	...
	LRESULT lRes = 0;
	if (uChildRes != 0)		//古月：应该是判断是否子窗口，这里为否——不是子窗口
	{
