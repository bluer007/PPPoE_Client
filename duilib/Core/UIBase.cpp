...
//////////////////////////////////////////////////////////////////////////
///
CWindowWnd::CWindowWnd() : m_hWnd(NULL), m_OldWndProc(::DefWindowProc), m_bSubclassed(false)			//古月：m_OldWndProc为系统自带的窗口函数
{
}
...
{
    if( GetSuperClassName() != NULL && !RegisterSuperclass() ) return NULL;			//古月：一般用户不会重写虚函数GetSuperClassName（）（默认返回NULL），所以就不会执行RegisterSuperclass（）来注册超类。
    if( GetSuperClassName() == NULL && !RegisterWindowClass() ) return NULL;		//古月：GetSuperClassName（）函数作为虚函数，应该是让用户来用自定义的类实现超类化，由于一般用户不会超类化，所以就调用默认的GetSuperClassName（）来返回一句NULL
    m_hWnd = ::CreateWindowEx(dwExStyle, GetWindowClassName(), pstrName, dwStyle, x, y, cx, cy, hwndParent, hMenu, CPaintManagerUI::GetInstance(), this);		//GetWindowClassName()是纯虚函数，所以必须重写
    ASSERT(m_hWnd!=NULL);
    return m_hWnd;
}
...
HWND CWindowWnd::Subclass(HWND hWnd)
{
    ...
    m_bSubclassed = true;
    m_hWnd = hWnd;
	::SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LPARAM>(this));		//古月:Allows any pointer to be converted into any other pointer type.---reinterpret_cast < type-id > ( expression )----msdn----应该就是用来强制转换
    return m_hWnd;
}
...
void CWindowWnd::ShowWindow(bool bShow /*= true*/, bool bTakeFocus /*= false*/)
{
    ASSERT(::IsWindow(m_hWnd));
    if( !::IsWindow(m_hWnd) ) return;
    ::ShowWindow(m_hWnd, bShow ? (bTakeFocus ? SW_SHOWNORMAL : SW_SHOWNOACTIVATE) : SW_HIDE);		//古月：showwindow的该函数纯属让指定窗口安照自定义show出来，且不会禁止父窗口的各种输入，而窗口关闭后不会专门去激活父窗口
}
...
UINT CWindowWnd::ShowModal()
{
    ...
    while( ::IsWindow(m_hWnd) && ::GetMessage(&msg, NULL, 0, 0) ) {
        if( msg.message == WM_CLOSE && msg.hwnd == m_hWnd ) {		//古月：ShowModal的该函数让指定窗口安正常模式show出来,且会禁止所有父窗口的鼠标及键盘输入，而窗口关闭后会专门去激活父窗口，
            nRet = msg.wParam;
...
bool CWindowWnd::RegisterWindowClass()		//古月：不是虚函数来的
{
    WNDCLASS wc = { 0 };
    wc.style = GetClassStyle();			//古月:虚函数，默认返回0，只有一句而已
    wc.cbClsExtra = 0;
    wc.cbWndExtra = 0;
    wc.hIcon = NULL;
    wc.lpfnWndProc = CWindowWnd::__WndProc;			//duilib中，一般是使用这个作为窗口过程，CWindowWnd::__ControlProc;	是给超类用的（CWindowWnd::RegisterSuperclass()有用到）
    wc.hInstance = CPaintManagerUI::GetInstance();
    wc.hCursor = ::LoadCursor(NULL, IDC_ARROW);
    wc.hbrBackground = NULL;
    wc.lpszMenuName  = NULL;
    wc.lpszClassName = GetWindowClassName();		//古月：纯虚函数，所以必须由用户来设定
    ATOM ret = ::RegisterClass(&wc);		//古月：注册窗口类了，而RegisterClassEx(&wc);是在CWindowWnd::RegisterSuperclass()中使用
    ASSERT(ret!=NULL || ::GetLastError()==ERROR_CLASS_ALREADY_EXISTS);
    return ret != NULL || ::GetLastError() == ERROR_CLASS_ALREADY_EXISTS;		//古月：||（或操作符）这种处理手段简洁
}
...
bool CWindowWnd::RegisterSuperclass()
{
    ...
    wc.lpfnWndProc = CWindowWnd::__ControlProc;		//CWindowWnd::__ControlProc；是给超类用的
    wc.hInstance = CPaintManagerUI::GetInstance();
...
LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( uMsg == WM_NCCREATE ) {		//古月：按照查询资料显示，WM_GETMINMAXINFO是窗口收到的第一条消息（在CPaintManagerUI中处理——保存窗口最大值最小值），WM_NCCREATE是收到的第二条消息
        ...
        if( uMsg == WM_NCDESTROY && pThis != NULL ) {		//古月：消息大全中显示WM_NCDESTROY是一个程序最后接收的消息
            LRESULT lRes = ::CallWindowProc(pThis->m_OldWndProc, hWnd, uMsg, wParam, lParam);	   //m_OldWndProc在CWindowWnd构造函数时候就初始化为系统默认窗口函数，而超类化和子类化会改变CWindowWnd的值
    ...
    if( pThis != NULL ) {
        return pThis->HandleMessage(uMsg, wParam, lParam);		//古月：CWindowWnd：：HandleMessage是虚函数，所以应该就调用用户重写的部分，当然可以不重写就会默认调用m_OldWndProc函数（初始化为系统默认窗口函数）
    } 
    else {
