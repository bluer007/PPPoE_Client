...
#include "stdafx.h"
#include <algorithm>			//古月：这是 WindowImplBase::IsInStaticControl(CControlUI *pControl)函数需要添加的，alberl教程所需
namespace DuiLib
{
...
LRESULT WindowImplBase::OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	::ScreenToClient(*this, &pt);		//古月：是屏幕坐标转化为客户区坐标，所以对于非客户区的坐标，转化后是负数

	RECT rcClient;
	::GetClientRect(*this, &rcClient);		//古月：GetClientRect——msdn—— The left and top members are zero. The right and bottom members contain the width and height of the window. 

	if( !::IsZoomed(*this) )	//古月:IsZoomed判断是否最大化
	{
	...
	////////////////////////////////////////////////////////////////////////////
	if (-1 == rcCaption.bottom)			//古月：两个///之间的是按照alberl教程添加，原来duilib没有，达到了只需要在xml中<Window>节点指定caption="0,0,0,-1"，那么不管窗口大小如何，都可以整个窗口拖动啦~
	{
...
BOOL WindowImplBase::IsInStaticControl(CControlUI *pControl)			//古月：该WindowImplBase::IsInStaticControl函数，是按照alberl教程添加的，就是用来判断pControl是否在静态控件中
{
	using namespace std;
	BOOL bRet = FALSE;
	if (! pControl)
	...
	strClassName.MakeLower();		//古月：MakeLower（）将strClassName转化为小写
	vctStaticName.push_back(_T("controlui"));		//古月：push_back（）为矢量插入元素
	...
	std::vector<CDuiString>::iterator it = std::find(vctStaticName.begin(), vctStaticName.end(), strClassName);		//古月：it为迭代器
